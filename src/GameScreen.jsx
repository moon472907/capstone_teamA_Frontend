...
function GameScreen({ onGoBack, selectedCharacter, gameId, playerId, accessToken, isSoundOn, onToggleSound }) {
  // ── 공통 UI 상태 ───────────────────────────────────────────
  ...
  // ── 게임 상태 ──────────────────────────────────────────────
  const [players,          setPlayers]          = useState(null); // 서버에서 받은 플레이어 목록
  const [currentPlayerIdx, setCurrentPlayerIdx] = useState(0);
  const [turn,             setTurn]             = useState(1);
  const [maxRounds,        setMaxRounds]        = useState(8);
  const [branchOptions,    setBranchOptions]    = useState(null); // tileId 배열
  ...
  // ── 두리버스 ───────────────────────────────────────────────
  const [busRideOptions, setBusRideOptions] = useState(null); // string[] | null
  ...
  const handleRequireBranchChoice = (opts) => {
    // 로컬 모드에서는 Phaser가 nodeId 문자열 배열로 전달
  ...
  const handleMoveDone = ({ isWin, nodeId }) => {
    ...
    if (nodeId && STAR_NODES.has(nodeId)) {
      ...
      if (scene) {
        scene.playStarPickup(nodeId, doAfterStar);
      } else {
        doAfterStar();
      }
      return;
